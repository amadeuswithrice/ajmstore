...
Class Master extends DBConnection {
	...
	function save_prodtype() {
		...
		// Retrieve ID and product type name from POST data
		$id = isset($_POST['id']) ? trim($_POST['id']) : '';
		$prod_type_name = isset($_POST['prod_type_name']) ? trim($_POST['prod_type_name']) : '';
	
		// Prepare data for the SQL query
		$data = "";
		foreach ($_POST as $k => $v) {
			if (!in_array($k, array('prod_type_ID'))) { // Exclude ID from the data being updated
				if (!is_numeric($v)) {
					$v = $this->conn->real_escape_string($v);
				}
				if (!empty($data)) $data .= ",";
		...
		// Check for duplicates based on prod_type_name
		if (!empty($id)) {
		...
		if ($check && $check->num_rows > 0) {
			// Duplicate found, update the existing product type instead of inserting
			$existingType = $check->fetch_assoc();
			$sql = "UPDATE `tbl_prod_type` SET {$data} WHERE prod_type_ID = '{$existingType['prod_type_ID']}'";
			error_log("Duplicate found. Updating existing product type with ID: {$existingType['prod_type_ID']}. Executing Update SQL: {$sql}");
		} else {
		...
		if ($save) {
			$gid = !empty($id) ? $id : $this->conn->insert_id;
			$resp['status'] = 'success';
			$resp['msg'] = empty($id) ? "Product Type has been successfully added." : "Product Type details have been updated successfully.";
		} else {
			$resp['status'] = 'failed';
	...
	function save_prodcat() {
		...
		// Retrieve ID and product category name from POST data
		$id = isset($_POST['id']) ? trim($_POST['id']) : '';
		$prod_cat_name = isset($_POST['prod_cat_name']) ? trim($_POST['prod_cat_name']) : '';
	
		// Prepare data for the SQL query
		$data = "";
		foreach ($_POST as $k => $v) {
			if (!in_array($k, array('id'))) { // Exclude ID from the data being updated
				if (!is_numeric($v)) {
					$v = $this->conn->real_escape_string($v);
				}
				if (!empty($data)) $data .= ",";
		...
		// Check for duplicates based on prod_cat_name
		if (!empty($id)) {
			$check = $this->conn->query("SELECT * FROM `prod_cat` WHERE `prod_cat_name` = '{$prod_cat_name}' AND id != '{$id}'");
		} else {
			$check = $this->conn->query("SELECT * FROM `prod_cat` WHERE `prod_cat_name` = '{$prod_cat_name}'");
		}
		...
		if ($check && $check->num_rows > 0) {
			// Duplicate found, update the existing product category instead of inserting
			$existingCategory = $check->fetch_assoc();
			$sql = "UPDATE `prod_cat` SET {$data} WHERE id = '{$existingCategory['id']}'";
			error_log("Duplicate found. Updating existing product category with ID: {$existingCategory['id']}. Executing Update SQL: {$sql}");
		} else {
			// No duplicates found, insert or update based on ID validity
			if (empty($id)) {
				// Insert operation if ID is empty
				$sql = "INSERT INTO `prod_cat` SET {$data}";
				error_log("ID is empty. Executing Insert SQL: {$sql}");
			} else {
				// Update operation if ID is valid
				$sql = "UPDATE `prod_cat` SET {$data} WHERE id = '{$id}'";
				error_log("No duplicates found. Executing Update SQL: {$sql}");
			}
		}
		...
		if ($save) {
			$gid = !empty($id) ? $id : $this->conn->insert_id;
			$resp['status'] = 'success';
			$resp['msg'] = empty($id) ? "Product category successfully added." : "Product category details updated successfully.";
		} else {
			$resp['status'] = 'failed';
			$resp['msg'] = "An error occurred: " . $this->conn->error . " [{$sql}]";
	...
	function save_supplier() {
		...
		// Retrieve ID and supplier name from POST data
		$id = isset($_POST['id']) ? trim($_POST['id']) : '';
		$suplier_name = isset($_POST['suplier_name']) ? trim($_POST['suplier_name']) : '';
		...
		// Check for duplicates based on suplier_name
		if (!empty($id)) {
		...
		if ($check && $check->num_rows > 0) {
			// Duplicate found, update the existing supplier instead of inserting
			$existingSupplier = $check->fetch_assoc();
			$sql = "UPDATE `supliers` SET {$data} WHERE suplier_id = '{$existingSupplier['suplier_id']}'";
			error_log("Duplicate found. Updating existing supplier with ID: {$existingSupplier['suplier_id']}. Executing Update SQL: {$sql}");
		} else {
			// No duplicates found, insert or update based on ID validity
			if (empty($id)) {
				// Insert operation if ID is empty
		...
		if ($save) {
			$gid = !empty($id) ? $id : $this->conn->insert_id;
			$resp['status'] = 'success';
			$resp['msg'] = empty($id) ? "Supplier successfully added." : "Supplier details updated successfully.";
		} else {
			$resp['status'] = 'failed';
			$resp['msg'] = "An error occurred: " . $this->conn->error . " [{$sql}]";
